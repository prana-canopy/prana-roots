...
export const MetricCard: React.FC<MetricCardProps> = ({ icon, label, value, trend, caption, index }: MetricCardProps) => {
   ...
   `}
      >
         ...
         <div className="relative z-10 flex flex-col h-full">
            {/* Top Section */}
            <div className="flex items-start justify-between mb-1 gap-1 sm:gap-2">  {/* Reduced gap on mobile */}
               {/* Label with adjusted size */}
               <div className={`text-[8px] sm:text-[8px] md:text-[10px] lg:text-xs font-medium ${theme === 'dark' ? 'text-white/70' : 'text-black/70'}`}>
                  {label}
               </div>

               {/* Icon container with overflow handling */}
               <div className={`p-0.5 sm:p-1 md:p-1 rounded-lg shrink-0 ${theme === 'dark' ? 'bg-white/10' : 'bg-black/10'}`}>
               ...
               <div className="relative">
                  <div className={`text-[8px] leading-none ${theme === 'dark' ? 'text-white/40' : 'text-black/40'} flex items-center gap-1`}>
                     ...
                     <Tooltip>
                      ...
                      `}
                           >
                              {label === "SEO" && "Perfect search engine optimization score, ensuring maximum visibility and reach for your target audience"}
